...
export default function EvidenceTable() {
  ...
  async function preview(id: string) {
    ...
    if (dataUrl) {
      ...
      if (w) {
        w.document.write(
          `<iframe src="${dataUrl}" style="width:100%;height:100%;border:0"></iframe>`
        );
      }
    }
  }
  ...
  async function download(item: Item) {
    ...
    a.href = dataUrl;
    a.download = item.name ?? "evidence-file";
  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        <div className="rounded-2xl bg-slate-900/50 p-6 text-sm text-slate-300">
          Loading...
        </div>
      ) : error ? (
        <div className="rounded-2xl bg-rose-900/20 p-6 text-sm text-rose-200">
          {error}
        </div>
      ) : (
            ...
            <tbody>
              {items.map((i) => (
                <tr key={i.id} className="border-t border-slate-800">
                  <td className="flex items-center gap-3 px-4 py-3 text-sm text-slate-200">
                    <FileIcon name={i.name ?? "unknown.file"} />

                    {i.name ?? "Unnamed file"}
                  </td>

                  <td className="px-4 py-3 text-sm text-slate-300">
                    {((i.size ?? 0) / 1024).toFixed(1)} KB
                  </td>
                  ...
                  <td className="flex gap-2 px-4 py-3 text-sm text-slate-300">
                    <button
                      onClick={() => preview(i.id)}
